...
export const attendAssign = async (req, res) => {
  try {
    ...
    if (req.body?.answers?.length == 0)
      return res
        .status(400)
        .json({ message: "please answer all the questions" });
    const view = await StudentAssignment.findOne({
      student: req.user.id,
    ...
    if (view?.scoredMarks !== 0 || view?.answers?.length !== 0)
      return res
        .status(400)
    ...
    await StudentAssignment.findByIdAndUpdate(
      { _id: view._id },
      { $set: { answers: req.body.answers, scoredMarks: score } },
      { new: true }
    );
    await Assignment.findByIdAndUpdate(
      { _id: view.assignment._id },
      { $push: { attendedStudents: view.student._id } },
      { new: true }
    );
...
export const stdViewTheirParticularAssign = async (req, res) => {
  try {
    if (!req.params.id)
      return res.status(400).json({ message: "please enter assignment id" });
    const view = await StudentAssignment.findOne({
      student: req.user.id,
      assignment: req.params.id,
    })
...
export const factViewTheirParticularAssign = async (req, res) => {
  try {
    if (!req.query.stdid)
      return res
        .status(400)
        .json({ message: "please enter student id in query" });
    if (!req.query.assignid)
      return res
        .status(400)
        .json({ message: "please enter assignment id in query" });
    const view = await StudentAssignment.findOne({
      student: req.query.stdid,
      assignment: req.query.assignid,
    })
      .populate({ path: "student", model: "User" })
      .populate({ path: "assignment", model: "Assignment" });
    if (!view) return res.status(404).json({ message: "assignment not found" });
