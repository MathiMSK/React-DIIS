...
export const createAssign = async (req, res) => {
  ...
  if (!selectedClass)
    return res.status(400).json({ message: "please enter class " });
  if (!req.body.assignmentTitle)
    return res.status(400).json({ message: "please enter assignment title" });
  if (!req.body.totalMarks)
    return res.status(400).json({ message: "please enter totalMarks" });
  if (!req.body.subject)
    return res.status(400).json({ message: "please enter subject" });
  let found = await Assignment.findOne({
  ...
  try {
    ...
    let getAllStudent = await User.find({
      isFaculty: false,
      class: selectedClass,
    });
